...
#era dataset
nce = Dataset('/srv/ccrc/data06/z5147939/ncfiles/clust_hr/tnn_SON_K_9_sil_0.1.nc', mode='r')
...
#print 'cluster 6 mean = %s, lat = %s, lon = %s' % (np.mean(sil[c6]), mlat[5], mlon[5])
#print 'cluster 7 mean = %s, lat = %s, lon = %s' % (np.mean(sil[c7]), mlat[6], mlon[6])
#print 'cluster 8 mean = %s, lat = %s, lon = %s' % (np.mean(sil[c8]), mlat[7], mlon[7])
#print 'cluster 9 mean = %s, lat = %s, lon = %s' % (np.mean(sil[c9]), mlat[8], mlon[8])
...
#awap k=9 data
nca = Dataset('/srv/ccrc/data06/z5147939/ncfiles/clust_hr/tnn_ap_SON_K_9_sil_0.1.nc', mode='r')
...
ax.annotate('9.', xy = (mlon[2],mlat[2]), xytext=(5, 5), textcoords='offset points')
m.plot(mlona[2],mlata[2], marker='o', color='g')
...
m.plot(mlona[4],mlata[4], marker='o', color='w', label='AWAP')

m.plot(mlon[4],mlat[4], marker='D', color='c')
ax.annotate('7.', xy = (mlon[4],mlat[4]), xytext=(5, 5), textcoords='offset points')
m.plot(mlona[3],mlata[3], marker='o', color='c')

m.plot(mlon[5],mlat[5], marker='D', color='m')
ax.annotate('8.', xy = (mlon[5],mlat[5]), xytext=(5, 5), textcoords='offset points')
...
ax.annotate('6.', xy = (mlon[7],mlat[7]), xytext=(5, 5), textcoords='offset points')
m.plot(mlona[7],mlata[7],'ko')

m.plot(mlon[8],mlat[8], marker='D', color='0.8')
ax.annotate('1.', xy = (mlon[8],mlat[8]), xytext=(5, 5), textcoords='offset points')
m.plot(mlona[8],mlata[8], marker='o', color='0.8')
...
plt.title('b) ERA-Interim SON TNn, K=9', fontsize=12)
...
cb = m.colorbar(mymap,"right", size="5%", pad="2%")
plt.title('c) AWAP SON TNn, K=9', fontsize=12)
cb.ax.tick_params(labelsize=8)
cb.set_label('Cluster mean silhouette coefficent', fontsize=10)
plt.savefig('/home/z5147939/hdrive/figs/tnn_son_9clust.png', bbox_inches='tight')
plt.show()
