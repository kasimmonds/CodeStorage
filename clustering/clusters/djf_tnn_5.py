...
##era dataset
#nce = Dataset('/home/z5147939/ncfiles/clust_era/tnn_DJF_K_5_sil_0.1.nc', mode='r')
...
##print 'cluster 4 mean = %s, lat = %s, lon = %s' % (np.mean(sil[c4]), mlat[3], mlon[3])
##print 'cluster 5 mean = %s, lat = %s, lon = %s' % (np.mean(sil[c5]), mlat[4], mlon[4])
##print stop
...
#for i in range(0,len(lon_c1)):
  #m.plot([mlon[0],lon_c1[i]],[mlat[0],lat_c1[i]], color='0.8')

#for i in range(0,len(lon_c2)):
  #m.plot([mlon[1],lon_c2[i]],[mlat[1],lat_c2[i]], color='0.8')

#for i in range(0,len(lon_c3)):
  #m.plot([mlon[2],lon_c3[i]],[mlat[2],lat_c3[i]], color='0.8')

#for i in range(0,len(lon_c4)):
  #m.plot([mlon[3],lon_c4[i]],[mlat[3],lat_c4[i]], color='0.8')

#for i in range(0,len(lon_c5)):
  #m.plot([mlon[4],lon_c5[i]],[mlat[4],lat_c5[i]], color='0.8')



#mymap= m.scatter(xi, yi, s=msize, c=s_clust, norm=norm, cmap=jet, edgecolors='none')
#medoid = m.plot(mmlon, mmlat, 'D', color='k', fillstyle='none', mew=2)
##annotate medoids according to strength
#ax.annotate('5.', xy = (mlon[0],mlat[0]), xytext=(5, 5), textcoords='offset points')
...
#ax.annotate('1.', xy = (mlon[3],mlat[3]), xytext=(5, 5), textcoords='offset points')
#ax.annotate('4.', xy = (mlon[4],mlat[4]), xytext=(5, 5), textcoords='offset points')


#cb = m.colorbar(mymap,"right", size="5%", pad="2%")
#plt.title('a) ERA-Interim DJF TNn, K=5', fontsize=10)
#cb.ax.tick_params(labelsize=8)
#cb.set_label('Cluster mean silhouette coefficent', fontsize=10)

#awap k=9 data
nca = Dataset('/srv/ccrc/data06/z5147939/ncfiles/clust_hr/tnn_DJF_K_8_sil_0.1.nc', mode='r')
sila = nca.variables['sil_width'][:,:]
clusta = nca.variables['cluster'][:,:]
mlona = nca.variables['medoid_lon'][:]
mlata = nca.variables['medoid_lat'][:]
lon = nca.variables['longitude'][:]
lat = nca.variables['latitude'][:]
...
#print 'cluster 0 mean = %s, lat = %s, lon = %s' % (np.mean(sila[c1a]), mlata[0], mlona[0])
#print 'cluster 1 mean = %s, lat = %s, lon = %s' % (np.mean(sila[c2a]), mlata[1], mlona[1])
#print 'cluster 2 mean = %s, lat = %s, lon = %s' % (np.mean(sila[c3a]), mlata[2], mlona[2])
#print 'cluster 3 mean = %s, lat = %s, lon = %s' % (np.mean(sila[c4a]), mlata[3], mlona[3])
#print 'cluster 4 mean = %s, lat = %s, lon = %s' % (np.mean(sila[c5a]), mlata[4], mlona[4])
#print 'cluster 5 mean = %s, lat = %s, lon = %s' % (np.mean(sila[c6a]), mlata[2], mlona[2])
#print 'cluster 6 mean = %s, lat = %s, lon = %s' % (np.mean(sila[c7a]), mlata[3], mlona[3])
#print 'cluster 7 mean = %s, lat = %s, lon = %s' % (np.mean(sila[c8a]), mlata[4], mlona[4])
...
ax.annotate('7.', xy = (mlona[1],mlata[1]), xytext=(5, 5), textcoords='offset points')
ax.annotate('2.', xy = (mlona[2],mlata[2]), xytext=(5, 5), textcoords='offset points')
ax.annotate('3.', xy = (mlona[3],mlata[3]), xytext=(5, 5), textcoords='offset points')
ax.annotate('6.', xy = (mlona[4],mlata[4]), xytext=(5, 5), textcoords='offset points')
ax.annotate('4.', xy = (mlona[5],mlata[5]), xytext=(5, 5), textcoords='offset points')
ax.annotate('5.', xy = (mlona[6],mlata[6]), xytext=(5, 5), textcoords='offset points')
ax.annotate('8.', xy = (mlona[7],mlata[7]), xytext=(5, 5), textcoords='offset points')


plt.title('b) ERA-Intermin DJF TNn, K=8', fontsize=12)
cb.ax.tick_params(labelsize=8)
cb.set_label('Cluster mean silhouette coefficent', fontsize=10)
plt.savefig('/home/z5147939/hdrive/figs/tnn_djf_8clust.png', bbox_inches='tight')
plt.show()
