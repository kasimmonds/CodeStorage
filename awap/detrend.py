...
#ssta dataset
had_djf = Dataset('/srv/ccrc/data06/z5147939/ncfiles/sstanom/HadISST_sst.21022017.nc.setctomiss.nc', mode='r')
de = Dataset('/srv/ccrc/data06/z5147939/ncfiles/sstanom/sst_detrend.nc', mode='r')
lon = had_djf.variables['longitude'][:]
lat = had_djf.variables['latitude'][:]
sst = had_djf.variables['sst'][:,:,:]
sst_de = de.variables['sst'][:,:,:]
time = had_djf.variables['time'][:]

sst_d = np.zeros((sst.shape))
for i in range(0,len(lon)):
    for j in range(0,len(lat)):
        sst_d[:,j,i] = signal.detrend(sst[:,j,i],axis=0)
sst_a = sst - sst_d
plt.figure(1)
plt.contourf(sst_d[0,:,:])
plt.show()
print stop

print sst.shape
print np.max(sst), np.min(sst)
print np.max(sst_d), np.min(sst_d)
print stop
