...
#AWAP
#DJF
awap_djf = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/txx_ap_3m_DJF.nc', mode='r')

lon = awap_djf.variables['lon'][:]
lat = awap_djf.variables['lat'][:]
adjf = awap_djf.variables['txx'][:,:,:]
time_units = awap_djf.variables['time'].units
time = awap_djf.variables['time'][:]

jra_djf = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/txx_jra_3m_DJF.nc', mode='r')
edjf = jra_djf.variables['txx'][:,:,:]
...
#correlations and pvalues for both jra and awap
...
#statistically significant correlations for jra and awap
ess_djf = np.zeros((len(lat), len(lon)))
ass_djf = np.zeros((len(lat), len(lon)))

#difference between jra and awap correlations
...
##MAM
#awap_mam = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/txx_ap_MAM.nc', mode='r')
#amam = awap_mam.variables['txx'][:,:,:]
#times = awap_mam.variables['time'][:]

#jra_mam = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/txx_jra_MAM.nc', mode='r')
#emam = jra_mam.variables['txx'][:,:,:]

#nmam = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/nino_jra_MAM.nc', mode='r')
#nino_mam = nmam.variables['sst'][:]


##write the loops for correlation with nino34, then take difference between awap and jra-interim 
...
##JJA
#awap_jja = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/txx_ap_JJA.nc', mode='r')
#ajja = awap_jja.variables['txx'][:,:,:]


#jra_jja = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/txx_jra_JJA.nc', mode='r')
#ejja = jra_jja.variables['txx'][:,:,:]

#njja = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/nino_jra_JJA.nc', mode='r')
...
#SON
awap_son = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/txx_ap_3m_SON.nc', mode='r')
ason = awap_son.variables['txx'][:,:,:]

jra_son = Dataset('/srv/ccrc/data06/z5147939/ncfiles/ds_comp_hr/txx_jra_3m_SON.nc', mode='r')
eson = jra_son.variables['txx'][:,:,:]
...
mymap= m.pcolormesh(xi, yi, ddjf, norm=norm, cmap=plt.cm.bwr)
...
print np.ma.max(dson), np.ma.min(dson)
...
print np.ma.max(ecorrson), np.ma.min(ecorrson)
...
cb.ax.tick_params(labelsize=6)
cb.set_label('Corr(ERA-Interim, NINO3.4)', fontsize=8)
plt.savefig('/home/z5147939/hdrive/figs/txx_jra_awap_n34.png', bbox_inches='tight')
plt.show()
