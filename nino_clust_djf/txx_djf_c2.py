...
medoid = m.plot(mlon[0], mlat[0], 'D', color='k', fillstyle='none', mew=2, markersize=3)
#cb = m.colorbar(mymap,"right", size="5%", pad="2%")
plt.title('TXx DJF Cluster 2', fontsize=10)
...
lat_idx_n1 = geo_idx(mlat[0], lat)
lon_idx_n1 = geo_idx(mlon[0], lon)
...
v = np.linspace( -80, 80, 17, endpoint=True)
norm = colors.BoundaryNorm(boundaries=v, ncolors=256)
...
v = np.linspace( -0.36, 0.36, 37, endpoint=True)
norm = colors.BoundaryNorm(boundaries=v, ncolors=256)
...
v = np.linspace( -420, 420, 22, endpoint=True)
norm = colors.BoundaryNorm(boundaries=v, ncolors=256)
...
plt.savefig('/home/z5147939/hdrive/figs/txx_djf_c2_enso.png', bbox_inches='tight')
#plt.show()
